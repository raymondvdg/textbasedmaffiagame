...
$name = ucwords(strtolower($name));
  ...
  if($checkifinfriends->rowCount() == 0) {
    print "This user is not on your friendslist.";
  } else {
    $SQL = $conn->prepare("DELETE from friendsTbl WHERE IDMAFFIA=:ownid AND IDFRIEND=:takeid"); 
    $SQL->execute(array('ownid'=> $id, 'takeid'=>$finalid));
      print "$name is removed from your friends list.";
  }
